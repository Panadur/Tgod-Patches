...
if (_action == 'repairAvionics') then
{
_partsToActOn = ["HitAvionics"];
_partsNeeded = 
	[
	["Exitem_electronics","Electronic Parts"],
...
{
_partsToActOn = ["HitHull","HitBatteries","HitMissiles","HitLight","HitHydraulics","HitTransmission","HitGear","HitHStabilizerL1","HitHStabilizerR1","HitVStabilizer1","HitTail","HitPitotTube","HitStaticPort","HitStarter1","HitStarter2","HitStarter3","HitWinch"];
_partsNeeded = 
	[
...
if (_action == 'repairMainRotor') then
{
	...
	_partsNeeded = 
		[
		["Exitem_rotor", "Main Rotor Assembly"]
		];
	_itemsNeeded = 
		[
...
if (_action == 'salvageAvionics') then
{
_partsToActOn = ["HitAvionics"];
_partsNeeded = 
	[
...
if (_action == 'repairHeloHull' || _action == 'repairEngine' || _action == 'repairAvionics' || _action == 'salvageAvionics'|| _action == 'repairTailRotor'|| _action == 'salvageTailRotor' || _action == 'salvageMainRotor' || _action == 'repairMainRotor' ||_action == 'repairCarEngine' || _action == 'salvageEngine' || _action == 'salvageCarEngine' ||  _action == 'repairCarHull') then
{
	if(_itemAction == 0) then
	{
