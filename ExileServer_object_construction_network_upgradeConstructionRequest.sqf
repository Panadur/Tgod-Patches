...
try
{
	...
	{
//	diag_log "i got a fortification upgrade in my inventory!!!";
	};
...
//	diag_log format ["network upgrade request: _objectConfig %1",_objectConfig];
 	if (_objectConfig isEqualTo []) then 
	{
 		throw "Invalid Upgrade";
	};
 	_objectConfig = _objectConfig select 0;
//	diag_log format ["network upgrade request: _objectConfig2 %1",_objectConfig];
 	_objectClass = getText (_objectConfig >> "upgradeObject");
//	diag_log format ["network upgrade request: _objectClass %1",_objectClass];
  ...
  // [_sessionID,"upgradeConstructionResponse",[_newObject]] call ExileServer_system_network_send_to; <-- disabled due to the change in Cryzy Mike's message format
 	[_sessionID,"upgradeConstructionResponse",[_newObject,[]]] call ExileServer_system_network_send_to; <-- added an Array to the message as the client awaits "STRING","ARRAY" as parameters in the message.
	//diag_log format ["network upgrade request: sessionID %1 , _newObject: %2",_sessionId,_newObject];
}
catch
{
